...
## 1. Merge the training and the test sets to create one data set:
    ...
    activities_test <- read.table ("y_test.txt"); activities_train <- read.table ("y_train.txt")
    #Combine test and train activity labels by row into a single data frame 
...
## 2. Extract only the measurements on the mean and standard deviation for each measurement: 
    ...
    features <- read.table ("features.txt",stringsAsFactors=FALSE, col.names=c("index", "features_label"))
    #Create vector that contains just the label column from the "features" data frame
    features_label <- features$features_label
    #Add logical vector to label data frame indicating columns which have mean() and std() in their name
    features_subset <- grepl('mean\\(\\)|std\\(\\)',features_label)
    features<- as.character(features_label[features_subset])
    #Add labels to x_set data frame before subsetting
    colnames(x_set) <- features_label          
...
## 3.Uses descriptive activity names to name the activities in the data set
    colnames(activities) <- "activity_code"
    ...
    activities_desc <- join(activities,activity_labels, by = "activity_code")
